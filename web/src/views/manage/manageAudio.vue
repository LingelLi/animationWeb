...
<div class='AudioManage'>
  ...
  <el-table
    ref="filterTable"
    :data="audioTableData.filter(data => data.Author === this.username &&( data.Name.toLowerCase().includes(search.toLowerCase())))"
    style="width: 100%">
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        ...
        @click="$router.push(`/AudioDetail/${scope.row._id}`)"></el-button>
        <el-button type="primary" size="small" icon="el-icon-edit"
        @click="$router.push(`/EditAudio/${scope.row._id}`)"></el-button>
  ...
  methods: {
    async handleCreate() {
      ...
      if(user.data.Upload_permission == "有上传权限"){
        this.$router.push('/manage/uploadAudio')
      }else{
        alert("您暂无上传权限");
        return false
      }
