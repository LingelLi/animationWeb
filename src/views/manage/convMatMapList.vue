<template>
    <div>
        ...
        <el-table :data="contentList.filter(data => (!search || data.valName.includes(search)) || data.keyNameList[0].toString().includes(search))" style="width: 100%;margin: 10px 0 0 30px">
            <el-table-column prop="valName" label="素材名称" width="250px">
        ...
        <div style="margin-bottom: 20px;">
            <el-select v-model="create_value" placeholder="请选择或输入素材名称" allow-create default-first-option filterable clearable @blur="oninput($event)" @change="onchange($event)">
        ...
        <div>
            ...
            <el-button v-else class="button-new-tag" size="small" @click="showInput">新增通用名称</el-button>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getAllValue(){
            await this.$http.get(`Actor`).then(res => {
                let data = res.data
                for(let d=0;d<data.length;d++){
                    if(!this.all_value.includes(data[d].Name)){
                        this.all_value.push(data[d].Name)
                    }
                }
            })
        },
        ...
        onchange(e){
            ...
            for(let i=0;i<this.contentList.length;i++){
                if(this.contentList[i].valName == e){
        ...
        handleCreate() {
            ...
            let find = false
            for(let i=0;i<this.contentList.length;i++){
                if(this.contentList[i].valName == this.create_value){
            ...
            if(!find){
                this.contentList.push({"valName": this.create_value, "keyNameList": [this.create_keys]})
            }
            this.rawdata.contentList = this.contentList
            this.$http.put(`convMatMapList/${this.rawdata._id}`, this.rawdata).then(res => {
