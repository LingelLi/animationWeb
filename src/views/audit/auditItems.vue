<template>
  <div class="ItemsManage">
    ...
    <el-table
      ref="filterTable"
      :data="ItemsTableData.filter(data => (( !searchForm.search || data.Name.toLowerCase().includes(searchForm.search.toLowerCase())) &&
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="mini"
            icon="el-icon-search"
            @click="$router.push(`/ItemsDetail/${scope.row._id}`)"
          />
  ...
  methods: {
    ...
    async audit(row) {
      this.$confirm('是否确定审核通过?', '提示', {
        ...
        const res = await this.$http.get(`Items/${row._id}`)
        this.change = res.data
        this.change.AuditStatus = '审核通过'
        this.$http.put(`Items/${row._id}`, this.change)
        this.$message({
          type: 'success',
          message: '审核通过!'
        })
