<template>
  <div class="OthersManage">
    ...
    <el-table
      ref="filterTable"
      :data="OthersTableData.filter(data => (( !searchForm.search || data.Name.toLowerCase().includes(searchForm.search.toLowerCase())) &&
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="mini"
            icon="el-icon-search"
            @click="$router.push(`/OthersDetail/${scope.row._id}`)"
          />
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      const res = await this.$http.get('Others')
      this.OthersTableData = res.data
    ...
    async audit(row) {
      this.$confirm('是否确定审核通过?', '提示', {
        ...
        const res = await this.$http.get(`Others/${row._id}`)
        this.change = res.data
        this.change.AuditStatus = '审核通过'
        this.$http.put(`Others/${row._id}`, this.change)
        this.$message({
          type: 'success',
          message: '审核通过!'
        })
    ...
    async remove(row) {
      this.$confirm('是否确定删除该素材?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        const res = await this.$http.delete(`Others/${row._id}`)
        this.$message({
          type: 'success',
          message: '删除成功!'
        })
