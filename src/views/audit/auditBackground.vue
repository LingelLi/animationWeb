<template>
  <div class="BackgroundManage">
    ...
    <el-table
      ref="filterTable"
      :data="backgroundTableData.filter(data => (( !searchForm.search || data.Name.toLowerCase().includes(searchForm.search.toLowerCase())) &&
  ...
  methods: {
    ...
    async audit(row) {
      this.$confirm('是否确定审核通过?', '提示', {
        ...
        const res = await this.$http.get(`Background/${row._id}`)
        this.change = res.data
        this.change.AuditStatus = '审核通过'
        this.$http.put(`Background/${row._id}`, this.change)
        this.$message({
          type: 'success',
          message: '审核通过!'
        })
